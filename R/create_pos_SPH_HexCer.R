...
#'
#' @export
create_pos_HexCer <- function(lipid_info, adduct, template = NA, ...) {
  ...
  if(is.na(template) & adduct == "[M+H]+") {
    template <- .template_pos_HexCer_MH()
  } #else if(is.na(template) & adduct == "[M+Na]+") {
    #template <- .template_pos_HexCer_MNa()
  #}
...
#'
#'
.template_pos_HexCer_MH <- function() {
  
  template <- list(
    "adduct_mass" = 5,
    "adduct_mass - water_mass" = 5,
    "adduct_mass - hexose_mass" = 150,
...
#'
#'
.template_pos_HexCer_MNa <- function() {
...
#'
#' @export
buildingblocks_pos_HexCer<- function() {
  
  building_blocks <- c("adduct_mass", "sphingoid_mass", "acyl_mass",
                       "hexose_mass", "proton_mass", "sodium_ion_mass")
