...
#'
#' @export
create_pos_DG <- function(lipid_info, adduct, template = NA, ...) {
  ...
  if(is.na(template) & adduct == "[M+Na]+") {
    template <- .template_pos_DG_MNa()
  } else if(is.na(template) & adduct == "[M+NH4]+") {
    template <- .template_pos_DG_MNH4()
  }
...
#'
#'
.template_pos_DG_MNH4 <- function() {
  
  template <- list(
    "adduct_mass" = 999,
    "adduct_mass - water_mass - rcdk::get.formula('NH3')@mass" = 200,
    "adduct_mass - sn1_mass - rcdk::get.formula('NH3')@mass" = 300,
    "adduct_mass - sn2_mass - rcdk::get.formula('NH3')@mass" = 300
  )
...
#'
#' @export
buildingblocks_pos_DG <- function() {
