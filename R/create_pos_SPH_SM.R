...
#'
#' @export
create_pos_SM <- function(lipid_info, adduct, template = NA, ...) {
  ...
  if(is.na(template) & adduct == "[M+H]+") {
    template <- .template_pos_SM_MH()
  } else if(is.na(template) & adduct == "[M+Na]+") {
    template <- .template_pos_SM_MNa()
  }
...
#'
#'
.template_pos_SM_MNa <- function() {
  
  template <- list(
    "adduct_mass" = 150,
    "adduct_mass - rcdk::get.formula('C3H9N')@mass" = 850,
    "adduct_mass - pc_mass" = 999
  )
