#' Generation of PC-O MS2 spectrum
#'
...
#'
#' @export
create_pos_PCO <- function(lipid_info, adduct, template = NA, ...) {
  ...
  if(is.na(template) & adduct == "[M+H]+") {
    template <- .template_pos_PCO_MH()
  } else if(is.na(template) & adduct == "[M+Na]+") {
    template <- .template_pos_PCO_MNa()
  }
