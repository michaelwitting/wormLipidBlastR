...
#'
#' @export
create_neg_PC_MFAH <- function(lipid_info, template = NA, ...) {
  ...
  adduct_mass <- lipidomicsUtils::calc_adduct_mass(lipid_mass, "[M+FA-H]-")
  ...
  mcols(lipidSpectrum)$precursorType <- "[M+FA-H]-"
...
#' Generation of fragmentation spectra for PC [M+HAc-H]-
#' 
#' @import MSnbase
#' @import S4Vectors
#'
#' @export
create_neg_PC_MHAcH <- function(lipid_info, template = NA, ...) {
  ...
  # calculate adduct mass ------------------------------------------------------
  adduct_mass <- lipidomicsUtils::calc_adduct_mass(lipid_mass, "[M+HAc-H]-")
  ...
  mcols(lipidSpectrum)$ionMode <- "NEGATIVE"
  mcols(lipidSpectrum)$precursorMz <- adduct_mass
  mcols(lipidSpectrum)$precursorType <- "[M+HAc-H]-"
  mcols(lipidSpectrum)$splash <- splash
  mcols(lipidSpectrum)$numPeak <- peaksCount(ms2Spec)
...
#'
#' @export
buildingblocks_neg_PC_MFAH <- function() {
...
#'
#' @export
buildingblocks_neg_PC_MHAcH <- function() {
