...
#'
#' @export
create_pos_PE_MH <- function(lipid_info, template = NA, ...) {
...
#' Generation of PC [M+Na]+ MS2 spectrum
#'
#' @import MSnbase
#' @import S4Vectors
#'
#' @export
create_pos_PE_MNa <- function(lipid_info, template = NA, ...) {
  ...
  # calculate adduct mass ------------------------------------------------------
  adduct_mass <- lipidomicsUtils::calc_adduct_mass(lipid_mass, "[M+Na]+")
  ...
  mcols(lipidSpectrum)$ionMode <- "POSITIVE"
  mcols(lipidSpectrum)$precursorMz <- adduct_mass
  mcols(lipidSpectrum)$precursorType <- "[M+Na]+"
  mcols(lipidSpectrum)$splash <- splash
  mcols(lipidSpectrum)$numPeak <- peaksCount(ms2Spec)
...
#'
#' @export
buildingblocks_pos_PE_MH <- function() {
